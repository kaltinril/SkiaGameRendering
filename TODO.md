...
| Godot           | 4.7.2   | Metal | Desktop (macOS) | —              | Not started | Godot exposes `MTLDevice`/`MTLCommandQueue` through `GetDriverResource`, but this repo has no Core.Metal, and MoltenVK is no fallback because SkiaSharp's macOS native build has no Vulkan backend. |
