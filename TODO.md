...
| Godot           | 4.7.2   | Metal | Desktop (macOS) | —              | Not started | Godot exposes `MTLDevice`/`MTLCommandQueue` through `GetDriverResource`, but this repo has no Core.Metal; macOS projects pin `rendering_device/driver.macos` to `vulkan` (MoltenVK) for now. |
