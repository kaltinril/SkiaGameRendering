...
| Godot           | 4.7.2 (GodotSharp) | Vulkan (RenderingDevice) | Desktop | samples/Sample.Godot.VK/ | Working (Windows verified, incl. clean `--gpu-validation`; Linux/macOS expected, unrun) | See `docs/godot/quickstart.md`. No reflection: `RenderingDevice.GetDriverResource` is public. Godot 4.6+ defaults new Windows projects to D3D12 - the project must pin `rendering_device/driver` to `vulkan`. |
| Godot           | 4.7.2   | D3D12 / Metal / Compatibility (GL) | Desktop | —                | Not started | D3D12: the same `GetDriverResource` calls return `ID3D12Device`/`IDXGIAdapter`/`ID3D12CommandQueue`/`ID3D12Resource`, so a `SkiaGameRendering.Godot.D3D12` on `Core.D3D12` is the obvious next adapter (Godot tracks D3D12 resource states the way it tracks Vulkan layouts - same hand-back barrier needed). Metal: no Core library. Compatibility: `DisplayServer.WindowGetNativeHandle(OpenGLContext)` exposes the HGLRC/EGLContext, so a shared-context `Core.OGL` adapter is plausible but unexplored. |
...
6. `src/SkiaGameRendering.Godot.VK/` landed — the first adapter needing no reflection at all (Godot exposes its `VkInstance`/`VkPhysicalDevice`/`VkDevice`/`VkQueue`/queue family and any RD texture's `VkImage` through `RenderingDevice.GetDriverResource`). It added `VkImageLayoutTransitioner` to `Core.VK` because Godot's render graph tracks every texture's `VkImageLayout` itself and SkiaSharp cannot be asked which layout to leave an image in (see `SkiaGameRendering-Notes.md` section 11). Follow-ups: a `Godot.D3D12` adapter on `Core.D3D12` (Godot 4.6+ defaults new Windows projects to D3D12), publishing the NuGet package, and Linux/macOS runs. `tests/Tests.Godot.VK` drives the real engine against the sample when `GODOT_BIN` is set and skips otherwise; CI does not fetch Godot.
